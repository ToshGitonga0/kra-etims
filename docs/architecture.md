...
[`VscuClientConfig`](../src/config/types.ts) has no `environment` field because VSCU has no KRA-hosted
...
[`docs/kra-contract-verification.md`](kra-contract-verification.md) for sources.
...
src/
  ...
  vscu/       -> Re-exposes the OSCU domain services against a VSCU base
                 URL — see [`docs/vscu.md`](vscu.md) for why this is safe to do (same
  ...
  errors/     -> EtimsError base class and four concrete subclasses,
                 ...
                 [`docs/kra-contract-verification.md`](kra-contract-verification.md)).
...
- lets contract-verification tests target one service at a time
  ([`tests/unit/sales-save-schema.test.ts`](../tests/unit/sales-save-schema.test.ts), etc.);
...
KRA (see "Fiscal safety" in [`README.md`](../README.md)). Every other endpoint still has
...
see [`CONTRIBUTING.md`](../CONTRIBUTING.md) — but was intentionally scoped down here rather than
...
[`HttpClient.post(path, body, safety)`](../src/client/HttpClient.ts) takes an explicit `"read"` or
`"write"` tag per call. Only `"read"` calls
...
the connection dropped. See the "Fiscal safety" section of the
[`README.md`](../README.md).
