...
[`docs/kra-contract-verification.md`](docs/kra-contract-verification.md): **nothing gets added to
`src/` without a traceable citation to an official KRA document, recorded in that file.**

## Before you start

1. Read [`docs/kra-contract-verification.md`](docs/kra-contract-verification.md) and
...
3. If something in KRA's documentation is ambiguous, silent, or internally
   ...
   section of [`docs/kra-contract-verification.md`](docs/kra-contract-verification.md) for a real example),
...
- **One domain per file** under [`src/oscu/services/`](src/oscu/services/) — don't add a second,
  unrelated endpoint to an existing service file; create a new one and wire
  it into [`src/oscu/index.ts`](src/oscu/index.ts).
- **Endpoint paths only live in [`src/oscu/endpoints.ts`](src/oscu/endpoints.ts).** Never inline a
...
- **Zod validation is required for any new fiscal write** (anything that
  ...
  pattern in [`src/oscu/services/sales.ts`](src/oscu/services/sales.ts).
- **Money fields** must use `hasEtimsAmountPrecision`/`roundEtimsAmount`
  from [`src/utils/numbers.ts`](src/utils/numbers.ts) in any new Zod schema involving amounts.
- **Dates** must use `toEtimsDate`/`toEtimsDateTime`/`isValidEtimsDate*`
  from [`src/utils/dates.ts`](src/utils/dates.ts) — never `Date#toISOString()`.
- **No secrets in logs.** Anything sensitive gets added to
  `SENSITIVE_KEYS` in [`src/utils/redaction.ts`](src/utils/redaction.ts).
- Update [`docs/kra-contract-verification.md`](docs/kra-contract-verification.md) in the same PR as any new or
...
- Unit tests ([`tests/unit/`](tests/unit/)) run in plain `npm test`, must not make real
  ...
  [`tests/unit/item-save-schema.test.ts`](tests/unit/item-save-schema.test.ts) /
  [`sales-save-schema.test.ts`](tests/unit/sales-save-schema.test.ts) for the pattern).
- Integration tests ([`tests/integration/`](tests/integration/)) are opt-in via
...
See [`.github/workflows/release.yml`](.github/workflows/release.yml). Releases are triggered by pushing a
